...
class AnimatedGrid extends StatefulWidget {
  ...
  static AnimatedGridState of(BuildContext context, { bool nullOk = false }) {
    assert(context != null);
    assert(nullOk != null);
    final AnimatedGridState result = context.ancestorStateOfType(const TypeMatcher<AnimatedGridState>());
    ...
    throw FlutterError(
      ...
      '  https://docs.flutter.io/flutter/widgets/AnimatedGridState-class.html \n'
...
/// ...
/// AnimatedGrid(key: GridKey, ...);
/// ...
/// GridKey.currentState.insert(123);
/// ```
///
...
class AnimatedGridState extends State<AnimatedGrid> with TickerProviderStateMixin<AnimatedGrid> {
  ...
  void initState() {
    ...
    _gridDelegate = widget.gridDelegate;
  }
  ...
  // The entry is only actually removed from the GridView when the remove animation
  ...
  ///
  /// This method's semantics are the same as Dart's [Grid.insert] method:
  /// it increases the length of the Grid by one and shifts all items at or
  /// after [index] towards the end of the Grid.
  ...
  ///
  /// This method's semantics are the same as Dart's [Grid.remove] method:
  /// it decreases the length of the Grid by one and shifts all items at or
  /// before [index] towards the beginning of the Grid.
